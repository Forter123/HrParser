...
<div data-qa="resume-serp__resume">
  <a href="/resume/abc123def456?query=x">Иван В., Python-разработчик</a>
...
def _configure(monkeypatch):
    monkeypatch.setattr("app.sources.hh_scraper_source.settings.hh_vision_folder_id", "folder1")
    monkeypatch.setattr("app.sources.hh_scraper_source.settings.hh_vision_profile_id", "profile1")
...
def _install_fakes(monkeypatch, page, start_response=None, fail_start=False):
    if start_response is None:
        start_response = {"folder_id": "folder1", "profile_id": "profile1", "port": 19513}

    vision_client = FakeVisionClient(start_response, fail_start=fail_start)
    monkeypatch.setattr(
        "app.sources.hh_scraper_source.httpx.AsyncClient", lambda **kw: vision_client
    )

    browser = FakeBrowser(page)
    chromium = FakeChromium(browser)
    monkeypatch.setattr(
        "app.sources.hh_scraper_source.async_playwright", lambda: FakePlaywright(chromium)
    )
...
async def test_fetch_starts_vision_profile_connects_and_parses_results(db_session, monkeypatch):
    ...
    assert entry.sender_id == "abc123def456"
    assert entry.message_link == "https://hh.ru/resume/abc123def456"
    ...
    assert chromium.connect_calls == ["http://127.0.0.1:19513"]
    assert vision_client.get_calls[-1] == "http://127.0.0.1:3030/stop/folder1/profile1"
    assert browser.closed is True
    assert page.closed is True

    # `text` alone is silently ignored by hh.ru (confirmed live — falls back
    # to the entire unfiltered database). These params were captured from a
    # real manual search and must be present for the text filter to apply.
    assert len(page.goto_calls) == 1
    goto_url = page.goto_calls[0]
    assert "pos=full_text" in goto_url
    assert "logic=normal" in goto_url
    assert "order_by=relevance" in goto_url
...
async def test_already_seen_resume_is_not_returned_again(db_session, monkeypatch):
    _configure(monkeypatch)
    search = _search(db_session, keywords="python")
    db_session.add(SourceSeenEntry(source="hh", search_query_id=search.id, external_id="abc123def456"))
    db_session.commit()
...
async def test_captcha_detected_and_cleared_after_manual_solve(db_session, monkeypatch):
    _configure(monkeypatch)
    monkeypatch.setattr("app.sources.hh_scraper_source.settings.hh_scraper_captcha_wait_seconds", 1)
    search = _search(db_session, keywords="python")

    page = FakePage(
        [CAPTCHA_HTML, SAMPLE_HTML],
        wait_for_selector_outcomes=[False, True],
    )
    _install_fakes(monkeypatch, page)
    ...
    assert len(results) == 1
    assert results[0][1].sender_id == "abc123def456"
...
async def test_captcha_not_solved_in_time_gives_up_for_that_search(db_session, monkeypatch):
    ...
    page = FakePage(
        ...
        wait_for_selector_outcomes=[False, False],
    )
    _install_fakes(monkeypatch, page)
